  ...
  return (
    <a href={link} className="group shadow cursor-pointer" target="_blank">
      <div className={cn("h-fit md:h-[300px] border-8 border-teal relative", className)}>
        ...
        <p className="hidden group-hover:block transition-all ease-in-out delay-200 absolute p-4 pb-2 bottom-0 text-shadow-sm text-bone bg-teal w-full">{subtitle}</p>
      </div>
    </a>
  );
};
